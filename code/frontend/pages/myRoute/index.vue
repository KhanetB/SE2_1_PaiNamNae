                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <h5
                                            class="mb-2 font-medium text-gray-900"
                                        >
                                            ผู้โดยสาร ({{
                                                route.passengers.length
                                            }}
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        @click.stop="completedRoute(route.id)"
                                        class="px-4 py-2 text-sm text-white bg-green-600 rounded-md hover:bg-green-700"
                                    >
                                        ยืนยันการถึงที่หมาย
                                    </button>
                        ...
                        <div v-else class="divide-y divide-gray-200">
                            ...
                            >
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center">
                                            ...
                                            >
                                                ...
                                                <span
                                                    class="absolute px-2 py-1 mb-2 text-xs text-white -translate-x-1/2 bg-gray-800 rounded-md opacity-0 pointer-events-none bottom-full left-1/2 w-max group-hover:opacity-100"
                                                    >ผู้โดยสารยืนยันตัวตนแล้ว</span
                                                >
                                            </div>
                                        </div>
                                        <div class="flex">
                                        ...
                                        <StarRating
                                            ...
                                            :reviews="trip.passenger.reviews"
                                        />
                                    </div>
                                ...
                                >
                                    ...
                                    <button
                                        v-if="trip.status === 'pending_review'"
                                        @click.stop="
                                            openConfirmModal(trip, 'complete')
                                        "
                                        class="px-4 py-2 text-sm text-white bg-green-700 rounded-md hover:bg-green-800"
                                    >
...
function openConfirmModal(trip, action) {
    ...
    const MAP = {
        confirm: {
            title: "ยืนยันคำขอจอง",
            message: `ยืนยันคำขอของ "${trip.passenger.name}" ใช่หรือไม่?`,
            confirmText: "ยืนยันคำขอ",
            variant: "primary",
        },
        reject: {
            title: "ปฏิเสธคำขอจอง",
            message: `ปฏิเสธคำขอของ "${trip.passenger.name}" ใช่หรือไม่?`,
            confirmText: "ปฏิเสธ",
            variant: "danger",
        },
        delete: {
        ...
        complete: {
            ...
            message: `ยืนยันว่าส่ง "${trip.passenger.name}" ถึงที่หมายแล้ว?`,
...
async function handleConfirmAction() {
    ...
    try {
        if (action === "confirm")
            await updateBookingStatus(bookingId, "CONFIRMED", "ยืนยันคำขอแล้ว");
        else if (action === "reject")
            await updateBookingStatus(bookingId, "REJECTED", "ปฏิเสธคำขอแล้ว");
        else if (action === "complete")
            await updateBookingStatus(
                bookingId,
                "COMPLETED",
                "ยืนยันการเดินทางเสร็จสิ้นแล้ว",
            );
...
async function handleConfirmDropoff(id) {
    selectedTripId.value = null;
    await confirmDropoff(id);
