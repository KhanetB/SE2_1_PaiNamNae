                                        ...
                                        >
                                            ...
                                            <span
                                                v-else-if="
                                                    ...
                                                    trip.status === 'mytrip'
                                                "
                                                class="status-badge status-mytrip"
                                                >เส้นทางของฉัน</span
                                            >
                                            <span
                                                v-else-if="
...
async function handleConfirmTrip(trip) {
    try {
        await $api(`/bookings/${trip.id}/status`, {
            method: "PATCH",
            body: { status: "pending_review" },
        });
        toast.success(
            "ส่งคำขอยืนยันไปยังคนขับแล้ว",
            "กรุณารอคนขับยืนยันการเดินทาง",
        );
        await fetchMyTrips();
    } catch (error) {
        toast.error(
            "เกิดข้อผิดพลาด",
            error?.data?.message || "ไม่สามารถยืนยันการเดินทางได้",
        );
    }
}
...
const confirmDropoff = async (tripId) => {
    try {
        const res = await $fetch(
            `/bookings/${tripId}/passenger-confirm-dropoff`,
            {
                method: "PATCH",
