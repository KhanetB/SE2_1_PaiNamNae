                            ...
                            >
                                ...
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <TripStatusBadge :status="trip.status" context="passenger" />
                                        </div>
                                ...
                                <div class="flex items-center mb-4 space-x-4">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <StarRating :rating="trip.driver.rating" :reviews="trip.driver.reviews" />
                                    </div>
                                ...
                                <div class="flex justify-end space-x-3" :class="{ 'mt-4': selectedTripId !== trip.id }">
                                    ...
                                    >ยกเลิกการจอง</button>
                                    <button
                                        v-if="trip.status === 'completed' && !trip.hasReview &&  (new Date() - new Date(trip.completedAt)) <= 7 * 24 * 60 * 60 * 1000"
                                    ...
                                    <span v-if="trip.status === 'completed' && trip.hasReview" class="px-4 py-2 text-sm text-green-600 transition duration-200 border border-green-300 rounded-md hover:bg-green-50">
                                        คุณได้เขียนรีวิวนี้ไปแล้ว
                                    </span>
                                    <span v-if="trip.status === 'completed' && !trip.hasReview && (new Date() - new Date(trip.completedAt)) > 7 * 24 * 60 * 60 * 1000" class="px-4 py-2 text-sm text-gray-500 transition duration-200 border border-gray-300 rounded-md hover:bg-gray-50">
...
const {
    allTrips, isLoading, activeTab, tabs, filteredTrips, getTripCount,
    fetchMyTrips, confirmDropoff, cancelBooking, deleteReview,
...
async function openEditReviewModal(trip) {
    try {
        ...
        if (!review) { toast.error('ไม่พบข้อมูลรีวิว'); return }
        tripToReview.value = trip
...
async function handleReviewSubmit(reviewData) {
    ...
    try {
        ...
        toast.error('บันทึกไม่สำเร็จ', error?.response?._data?.message || 'เกิดข้อผิดพลาด')
    } finally {
...
function toggleTripDetails(tripId) {
    ...
    if (trip) googleMap.updateMap(trip)
...
onMounted(() => {
    googleMap.loadGoogleMaps(() => {
        fetchMyTrips(doReverseGeocode).then(() => {
            if (filteredTrips.value.length) googleMap.updateMap(filteredTrips.value[0])
        })
    })
})
</script>
