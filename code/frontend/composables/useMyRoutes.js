...
export function useMyRoutes() {
    ...
    function buildWaypointLists(r) {
        ...
        const stopsCoords = orderedList.map((p) =>
            p && typeof p.lat === 'number' && typeof p.lng === 'number'
                ? { lat: p.lat, lng: p.lng, name: p.name || '', address: p.address || '' } : null
        ).filter(Boolean)
    ...
    function buildCarDetails(vehicle) {
        ...
        if (Array.isArray(vehicle.amenities) && vehicle.amenities.length) d.push(...vehicle.amenities)
        return d
    }
    ...
    async function fetchMyRoutes(reverseGeocodeCallback) {
        ...
        try {
            ...
            const allowedStatuses = new Set(['AVAILABLE', 'FULL', 'IN_TRANSIT', 'COMPLETED'])
            const formatted = []
            const ownRoutes = []
            ...
            for (const r of routes) {
                ...
                ownRoutes.push({
                    ...
                    passengers: confirmedBookings.map((b) => ({
                        ...
                        email: b.passenger?.email || '',
                        isVerified: !!b.passenger?.isVerified,
                        rating: 4.5, reviews: Math.floor(Math.random() * 50) + 5,
                    })),
                })
            }
    ...
    async function updateBookingStatus(bookingId, status, successMsg) {
        await $api(`/bookings/${bookingId}/status`, { method: 'PATCH', body: { status } })
    ...
    async function completedRoute(routeId) {
        try {
            ...
            toast.error('เกิดข้อผิดพลาด', error?.response?._data?.message || `${error}`)
        }
    }
