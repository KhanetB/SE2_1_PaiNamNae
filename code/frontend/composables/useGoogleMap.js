  ...
  function reverseGeocode(lat, lng) {
    ...
    return new Promise((resolve) => {
      ...
      geocoder.geocode({ location: { lat, lng } }, (results, status) => {
        ...
        const result = status === "OK" && results?.length ? results[0] : null;
    ...
    return new Promise((resolve) => {
      ...
      placesService.getDetails(
        ...
        (place, status) => {
          ...
          // else resolve(null);
          const name =
            status === google.maps.places.PlacesServiceStatus.OK && place?.name
              ? place.name
              : null;
          placeCache.set(placeId, name);
          resolve(name);
        },
      );
    });
