<template>
    ...
    >
        <div class="w-4xl p-6 bg-white rounded-lg shadow-xl overflow-y-auto max-h-[90vh]">
            <h3 class="text-3xl font-semibold text-gray-900">รีวิวการเดินทาง</h3>
            <p class="mt-1 text-2xl font-semibold text-gray-700">
                {{ trip?.origin }} → {{ trip?.destination }}
            </p>
            <p class="mt-1 text-md text-gray-600">จุดนัดพบ: {{ trip?.pickupPoint }}</p>
            ...
            <div class="mt-8 justify-center content-center place-items-center">
                ...
                />
                <h5 class="font-medium text-xl text-gray-900 mt-3">{{ trip?.driver?.name }}</h5>
            ...
            <div class="mt-4">
                ...
                <div class="flex gap-3">
                    <div v-for="(url, index) in localPreviewImages" :key="index" class="relative w-48 h-48">
            ...
            <div class="flex justify-end gap-2 mt-6">
                ...
                >{{ isSubmitting ? 'กำลังส่ง...' : 'ส่งรีวิว' }}</button>
            </div>
        </div>
    </div>
</template>
...
const REVIEW_TAGS = ['ขับขี่ปลอดภัย', 'รถสะอาดน่านั่ง', 'คนขับอัธยาศัยดี', 'ชอบเพลงที่เปิด']
...
watch(() => props.show, (val) => {
    ...
    if (props.existingReview) {
        ...
        localSelectedTags.value = (props.existingReview.labels ?? [])
            .map(l => REVIEW_TAG_MAP_REVERSE[l]).filter(Boolean)
