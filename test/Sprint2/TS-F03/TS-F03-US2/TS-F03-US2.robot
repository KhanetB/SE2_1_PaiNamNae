*** Settings ***
Documentation   US2-การ anonymous ฝั่งผู้ใช้ที่เป็น Drivers
...
TC13 สร้าง Account Passenger และ Driver ผ่าน Web UI และเข้าหน้า Home
...
TC17 Driver สร้างเส้นทาง (Routes) ผ่าน Web UI
...
TC18 - TC21 Flow การจองและเดินทางแบบ Real-time (ซ้าย Passenger / ขวา Driver)
...
TC22, TC23 Driver ทำการลบบัญชีผู้ใช้
    [Documentation]    รวบ TC11 (Login) และ TC12 (Delete) เข้าด้วยกันตาม Flow ใช้งานจริง
    Switch Browser    driver_window
    Login To System    ${DRIVER_USER}    ${DRIVER_PASS}
    Deletes Account driver    ${DRIVER_EMAIL}    ${DRIVER_PASS}

TC24 Passenger เห็นข้อมูล Driver เป็น Anonymous
    Switch Browser    passenger_window
    Login To System    ${PASSENGER_USER}    ${PASSENGER_PASS}
...
TC10, TC11 Driver ทำการลบบัญชีผู้ใช้
    [Documentation]    รวบ TC11 (Login) และ TC12 (Delete) เข้าด้วยกันตาม Flow ใช้งานจริง
    Switch Browser    driver_window
    Login To System    ${DRIVER_USER}    ${DRIVER_PASS}
    Deletes Account driver    ${DRIVER_EMAIL}    ${DRIVER_PASS}

TC12 Passenger เห็นข้อมูล Driver เป็น Anonymous
    Switch Browser    passenger_window
    Login To System    ${PASSENGER_USER}    ${PASSENGER_PASS}
    Wait Until Element Is Visible    xpath=//a[@href='/myTrip']    timeout=10s
    Click Element                    xpath=//a[@href='/myTrip']
